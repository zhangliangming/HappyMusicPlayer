...
public class Copy_2_of_ScanMusicActivity extends Activity {
	/**
	 * 初始界面
	 */
		...
		if (files != null) {
			for (int i = 0; i < files.length; i++) {
				...
				if (f.isFile()) {
					...
					AsyncTaskMessageHandler asyncTaskHandler = new AsyncTaskMessageHandler();
					asyncTaskHandler.execute(f.getPath());
	...
	private class AsyncTaskMessageHandler extends
		...
		protected void onProgressUpdate(String... values) {
			scanPathTextView.setText(values[0]);
